...
def train (args, model, device, x, y, optimizer,criterion, task_id, epoch):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        if not model.buffer.is_empty():
            buf_inputs, buf_labels = model.buffer.get_data(args.batch_size_train)
            loss += criterion(model(buf_inputs), buf_labels)
        ...
        optimizer.step()
        model.buffer.add_data(examples=data_x, labels=data_y)
...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        for epoch in range(1, args.n_epochs+1):
            # Train
            clock0=time.time()
            train(args, model, device, xtrain, ytrain, optimizer, criterion, task_id, epoch)
            clock1=time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--buffer_size', type=int, default=1000,
                        help='number of buffer_size (default: 1000)')
    ...
    parser.add_argument('--savename', type=str, default='./log/PMNIST/ER/',
                        help='save path')
