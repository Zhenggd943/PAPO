...
def train_projected(args,model,device,x,y,optimizer,criterion,feature_mat,task_id, epoch, Lam):
    model.train()
    r=np.arange(x.size(0))
    np.random.shuffle(r)
    r=torch.LongTensor(r).to(device)
        ...
        for k, (m,params) in enumerate(model.named_parameters()):
            if len(params.size()) == 4 and 'weight' in m:
                ...
                if args.use_papo == 'True':
                    if epoch % args.interval == 0:
                        papo_grad = polar_express_(args, flat_grad)
                        final_grad = flat_grad - torch.mm(flat_grad,
                                                          feature_mat[kk]) + Lam * papo_grad  # args.Lambda * papo_grad
                    else:
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            for epoch in range(1, args.n_epochs+1):
                # Train 
                clock0=time.time()
                train_projected(args, model,device,xtrain, ytrain,optimizer,criterion,feature_mat,k,epoch, Lam)
                clock1=time.time()
                tr_loss, tr_acc = test(args, model, device, xtrain, ytrain,criterion,k)
                log.info('Epoch {:3d} | Train: loss={:.3f}, acc={:5.1f}% | time={:5.1f}ms |'.format(epoch,
                                                                                                    tr_loss,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--Lambda0', default=1, type=float,
                        help='coefficient lambda0 (default: 1)')
    parser.add_argument('--Lambda', default=0.05, type=float,
                        help='coefficient lambda (default: 0.05)')
    ...
    parser.add_argument('--savename', type=str, default='./log/FIVE/GPM',
                        help='save path')
