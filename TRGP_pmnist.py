...
def train (args, model, device, x, y, optimizer,criterion, epoch, task_id):
    model.train()
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for k, (m,params) in enumerate(model.named_parameters()):
            if args.use_papo == 'True' and  'weight' in m:
...
def train_projected_regime (args, model,device,x,y,optimizer,criterion, memory, task_name, task_name_list, task_id, feature_mat, epoch, space1=[None, None, None], space2=[None, None, None]):
    model.train()
    r=np.arange(x.size(0))
    np.random.shuffle(r)
    r=torch.LongTensor(r).to(device)
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            for epoch in range(1, args.n_epochs+1):
                # Train
                clock0=time.time()
                train(args, model, device, xtrain, ytrain, optimizer, criterion, epoch, task_id)
                clock1=time.time()
            ...
            for epoch in range(1, args.n_epochs+1):
                ...
                clock0=time.time()
                train_projected_regime(args, model,device,xtrain, ytrain,optimizer,criterion,memory, task_name, task_name_list, task_id, feature_mat, epoch, space1=space1, space2=space2)
                clock1=time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--Lambda0', default=0.01, type=float,
                        help='coefficient lambda0 (default: 0.5)')
    ...
    parser.add_argument('--savename', type=str, default='log/PMNIST/TRGP',
                        help='save path')
