...
def train_projected_regime (args, model, device,x,y,optimizer,criterion,memory, task_name, task_name_list, space_list_all, task_id, feature_mat, epoch, Lam, space1=[None, None, None], space2=[None, None, None]):
    model.train()
    r=np.arange(x.size(0))
    np.random.shuffle(r)
    r=torch.LongTensor(r).to(device)
        ...
        for k, (m,params) in enumerate(model.named_parameters()):
            if len(params.size())==4 and 'weight' in m:
                ...
                if args.use_papo == 'True':
                    if epoch % args.interval == 0:
                        papo_grad = polar_express_(args, flat_grad)
                        final_grad = flat_grad - torch.mm(flat_grad, feature_mat[kk]) + Lam * papo_grad #args.Lambda * papo_grad
                    else:
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            for epoch in range(1, args.n_epochs+1):
                ...
                clock0=time.time()
                train_projected_regime(args, model, device,xtrain, ytrain,optimizer,criterion,memory, task_name, task_name_list, space_list_all,task_id, feature_mat, epoch, Lam, space1=space1, space2=space2)
                clock1=time.time()
        ...
        # update task id 
        task_id +=1
        save_path = args.savename + '/' + str_time
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        model_path = save_path + '/model_random_' + str(args.seed) + '_task_' + str(task_id)
        save_model(model, memory, model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_papo', type=str, default='True',
                        help='True or False')
    parser.add_argument('--iteration_type', type=str, default='Polar_Express',
                        choices=['Newton_schulz', 'Polar_Express'], help='Type of iteration method')
    ...
    parser.add_argument('--Lambda', default=0.01, type=float,
                        help='coefficient lambda (default: 0.5)')
    ...
    parser.add_argument('--savename', type=str, default='./log/FIVE/TRGP',
                        help='save path')
    ...
    print_log('Accuracy: ' + str(accs), log)
    print_log('Backward transfer: ' + str(bwts), log)
    print_log('Final Avg Accuracy: {:5.2f}%, std:{:5.2f}'.format(np.mean(accs), np.std(accs)), log)
    print_log('Final Avg Backward transfer: {:5.2f}%, std:{:5.2f}'.format(np.mean(bwts), np.std(bwts)), log)
